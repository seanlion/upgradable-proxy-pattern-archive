...
contract('EternalStorageProxy', ([_, proxyOwner, tokenOwner, anotherAccount]) => {
  ...
  const initializeData = '0x' + methodId + params; // 함수 id랑 데이터를 hex로 가공해서 전달

  beforeEach(async function () {
    /* truffle contract 문법 
      new(): Deploy a new version of this contract to the network, 
      getting an instance of MyContract that represents the newly deployed instance.
    */
    proxy = await EternalStorageProxy.new({ from: proxyOwner }) // proxyOwner 권한으로 ESProxy 컨트랙트 생성
    impl_v0 = await Token_V0.new() // 실제 컨트랙트 생성
  ...
  describe('transferOwnership', function () {
    describe('when the new proposed owner is not the zero address', function () {
      ...
      describe('when the sender is the token owner', function () {
        ...
        // 버전 1로 업그레이드 할 때는 impl 컨트랙트로 업그레이드
      ...
      describe('when the sender is the proxy owner', function () {
        ...
        describe('when no initial version was provided', function () {
          it('upgrades to the given version', async function () {
            await proxy.upgradeTo('0', impl_v0.address, { from }) // 컨트랙트 주소만 변경
  ...
  describe('upgrade and call', function () {
    ...
    describe('when the new implementation is not the zero address', function () {
      ...
      describe('when the sender is the proxy owner', function () {
        ...
        it('upgrades to the given version', async function () {
          // impl 컨트랙트 주소 바꾸면서 initialize까지 실행
